...
export const notesHandlers = [
  ...
  // 特定のメモの情報を取得する
  rest.get<never, { noteId: string }, Note>("/notes/:noteId", (req, res, ctx) => {
    const { noteId } = req.params;
    return res(ctx.delay(1000), ctx.status(200), ctx.json({ ...EXAMPLE_NOTE, id: noteId }));
  }),

  // 特定のメモを更新する
  rest.put<string, { noteId: string }, Note>("/notes/:noteId", (req, res, ctx) => {
    const { noteId } = req.params;
  ...
  // 特定のメモを削除する
  rest.delete<never, { noteId: string }, Pick<Note, "id">>("/notes/:noteId", (req, res, ctx) => {
    const { noteId } = req.params;
    return res(ctx.delay(1000), ctx.status(200), ctx.json({ id: noteId }));
  }),

  // 特定のメモを公開する
  rest.patch<never, { noteId: string }, Note>("/notes/:noteId/public", (req, res, ctx) => {
    const { noteId } = req.params;
    return res(
      ctx.delay(1000),
      ctx.status(200),
  ...
  rest.get<never, { userId: string }, ListNote[]>("/users/:userId/notes", (req, res, ctx) => {
  ...
  rest.get<string, { userId: string; keyword: string }, ListNote[]>(
