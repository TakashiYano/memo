...
export const usersHandlers = [
  // ユーザーを作成する
  rest.post<string, never, UserType>("/users", (_req, res, ctx) => {
    return res(ctx.delay(1000), ctx.status(201), ctx.json(EXAMPLE_USER_01));
  }),
  // 特定のユーザーの情報を取得する
  rest.get<never, { userId: string }, UserType>("/users/:userId", (req, res, ctx) => {
    const { userId } = req.params;
    return res(ctx.delay(1000), ctx.status(200), ctx.json({ ...EXAMPLE_USER_01, id: userId }));
  }),
  // 特定のユーザー(自分)の情報を更新する
  rest.put<string, { userId: string }, UserType>("/users/:userId", (req, res, ctx) => {
    const { userId } = req.params;
